...
export default function KernelXPromptEngine() {
  ...
  return (
    <FlowField theme="aurora" density="medium">
      <div className="min-h-screen relative z-10 px-4 sm:px-6 py-6 md:py-8 max-w-6xl mx-auto flex flex-col justify-center">
        
        {/* Main Liquid Glass Shell Container */}
        <div className="w-full liquid-glass rounded-3xl p-6 sm:p-8 border border-white/20 shadow-2xl space-y-6 relative overflow-hidden">
          
          <div className="space-y-6 w-full">
            {/* Header */}
            <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-5 border-b border-white/15">
              <div>
                <div className="flex flex-wrap items-center gap-3">
                  <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight font-mono text-white drop-shadow-sm">
                    KernelX Prompt
                  </h1>
                  <Badge variant="outline" className="text-[11px] font-mono border-indigo-400 text-indigo-300 bg-indigo-500/20 px-2.5 py-0.5 font-bold">
                    v3.4 PRO
                  </Badge>
                </div>
              ...
              <div className="flex items-center gap-3 self-end sm:self-auto">
                <Button
                  ...
                  className="font-mono text-xs font-semibold gap-2 border-white/20 bg-white/10 hover:bg-white/20 text-white min-h-[38px] px-3.5"
                >
                    ...
                    {activeTab === 'audit' && (
                      <>
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-mono text-xs font-bold transition-all shadow-lg min-h-[40px] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                        ...
                        <button
                          ...
                          disabled={isProcessing || !prompt.trim()}
                          className="inline-flex items-center gap-2 px-5 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-mono text-xs font-bold transition-all min-h-[40px] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            <div className="relative overflow-hidden w-full">
              <AnimatePresence custom={direction} mode="wait">
                  ...
                  >
                    {analysisResult && (
                      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                        {/* Score Card */}
                        <LiquidGlassCard className="lg:col-span-4 flex flex-col items-center justify-center p-6 text-center space-y-3">
                          <span className="text-xs font-mono uppercase tracking-widest text-slate-300 font-bold">Overall Quality Score</span>
                          <div className="relative flex items-center justify-center w-28 h-28 sm:w-32 sm:h-32 rounded-full border-4 border-indigo-400 bg-indigo-500/20 shadow-xl shrink-0">
                            <span className={`font-mono text-3xl sm:text-4xl font-extrabold ${analysisResult.overallScore >= 80 ? 'text-emerald-400' : analysisResult.overallScore >= 50 ? 'text-amber-400' : 'text-red-400'}`}>
                        ...
                        <LiquidGlassCard className="lg:col-span-8 p-5 sm:p-6 space-y-4">
                          <h3 className="font-mono text-xs sm:text-sm font-bold tracking-wider text-white uppercase border-b border-white/15 pb-2.5">
                          ...
                          <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,180px),1fr))] gap-3 sm:gap-4">
                            {Object.entries(analysisResult.criteria).map(([key, val]) => (
                              <div key={key} className="p-3.5 rounded-xl bg-zinc-900/90 border border-white/15 space-y-2">
                                <div className="flex justify-between font-mono text-xs">
                                  <span className="capitalize font-bold text-slate-200 truncate max-w-[120px]">{key}</span>
                        ...
                        {securityScan && !securityScan.isSecure && (
                          <div className="lg:col-span-12">
                            <Alert variant="destructive" className="bg-red-950/80 border-red-500/50 text-white">
                              ...
                              <AlertTitle className="font-mono text-xs font-bold">Security Threat Detected</AlertTitle>
                        ...
                        {optimizedResult && (
                          <LiquidGlassCard className="lg:col-span-12 p-5 sm:p-6 space-y-4">
                            <div className="flex items-center justify-between border-b border-white/15 pb-3">
                              <div className="flex items-center gap-2 text-indigo-300">
                                <Sparkles className="h-4 w-4" />
                                <h3 className="font-mono text-sm font-bold text-white">KernelX Optimized Output</h3>
                              </div>
                              <div className="flex items-center gap-2">
                                <Button variant="outline" size="sm" onClick={() => setIsDiffModalOpen(true)} className="font-mono text-xs border-white/20 text-white hover:bg-white/10 min-h-[36px]">
                                  <Split className="h-3.5 w-3.5 mr-1 text-indigo-300" /> Compare Diff
                                </Button>
                                <Button variant="outline" size="sm" onClick={() => handleCopy(optimizedResult.optimizedPrompt)} className="font-mono text-xs border-white/20 text-white hover:bg-white/10 min-h-[36px]">
                                  <Copy className="h-3.5 w-3.5 mr-1 text-indigo-300" /> Copy
                                </Button>
                              </div>
                            </div>

                            <div className="p-4 rounded-xl bg-zinc-950/90 border border-white/15 font-mono text-xs sm:text-sm text-slate-100 leading-relaxed whitespace-pre-wrap">
                ...
                {activeTab === 'factory' && (
                  ...
                  >
                    {enhancementResult && (
                      <LiquidGlassCard className="p-5 sm:p-6 space-y-4">
                        <div className="flex items-center justify-between border-b border-white/15 pb-3">
                          <span className="font-mono text-sm font-bold text-white">Production-Grade Enhanced Prompt</span>
                          <Badge variant="outline" className="font-mono text-xs border-indigo-400 text-indigo-300 uppercase px-2.5 py-0.5 font-bold">
                ...
                {activeTab === 'vision' && (
                  ...
                  >
                    {!imagePreview ? (
                      ...
                      <div className="relative rounded-2xl overflow-hidden border border-white/20 glass-card p-3 sm:p-4 bg-zinc-950/80 shadow-2xl space-y-3">
                        <input
                          ref={imageInputRef}
                          type="file"
                          accept="image/*"
                        ...
                        <div className="relative w-full max-h-[300px] min-h-[180px] flex items-center justify-center overflow-hidden rounded-xl bg-black/90">
                          <img
                            src={imagePreview}
                          ...
                          <div className="absolute inset-0 flex items-center justify-center gap-3 bg-black/40 backdrop-blur-[2px] opacity-95 transition-opacity">
                            <button
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-900/90 hover:bg-zinc-800 border border-white/20 text-white font-mono text-xs font-bold shadow-xl transition-all active:scale-95 cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-950/90 hover:bg-red-900 border border-red-500/40 text-red-300 font-mono text-xs font-bold shadow-xl transition-all active:scale-95 cursor-pointer"
                            >
                        ...
                        {imageMeta && (
                          <div className="flex items-center justify-between px-1 font-mono text-xs text-slate-300">
                    ...
                    {visionResult && (
                      <LiquidGlassCard className="p-5 sm:p-6 space-y-4">
                        <div className="flex items-center justify-between border-b border-white/15 pb-3">
                          ...
                          <Button variant="outline" size="sm" onClick={() => handleCopy(visionResult.generatedPrompt)} className="font-mono text-xs border-white/20 text-white hover:bg-white/10 min-h-[36px]">
                            <Copy className="h-3.5 w-3.5 mr-1 text-indigo-300" /> Copy
                          </Button>
                        </div>

                        <div className="p-4 rounded-xl bg-zinc-950/90 border border-white/15 font-mono text-xs sm:text-sm text-slate-100 leading-relaxed">
          ...
          {/* Footer */}
          <footer className="pt-5 border-t border-white/15 text-center font-mono text-xs text-slate-300 font-medium">
